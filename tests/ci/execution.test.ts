...
import { fetchExecutionForOverlay } from '../../src/ci/execution';

const {
    sendMock,
    fetchExecutionHistoryMock,
    listExecutionsCommandMock,
    sfnClientMock,
} = vi.hoisted(() => ({
...
describe('fetchExecutionForOverlay', () => {
    ...
    it('filters by FAILED status for latest-failed mode', async () => {
        sendMock.mockResolvedValue({
            executions: [{ executionArn: 'arn:exec:run-2' }],
        });
        fetchExecutionHistoryMock.mockResolvedValue([]);

        await fetchExecutionForOverlay({
            mode: 'latest-failed',
    ...
    it('passes the region through to the SFN client when provided', async () => {
        sendMock.mockResolvedValue({
            executions: [{ executionArn: 'arn:exec:run-3' }],
        });
        fetchExecutionHistoryMock.mockResolvedValue([]);
