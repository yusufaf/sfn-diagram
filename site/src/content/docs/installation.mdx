...
The core package pulls in **no browser engine** — SVG and Mermaid generation stay lightweight. PNG export uses `@resvg/resvg-js`, a native, browser-free rasterizer, provided by an optional peer dependency. Install it only if you use `sfn-diagram/png`:
